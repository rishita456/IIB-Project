...
for gain in training_gains:
    for trial in trials:
        ...
        # plt.plot(time[500:len(time_og)-500],steps[500:len(time_og)-500], label = 'raw step data')
        # plt.plot(time[500:len(time_og)-500],y[500:len(time_og)-500], label = 'filtered step data')
        # plt.xlabel('time(s)')
        ...
        residuals = steps[500:len(time_og)-500] - y[500:len(time_og)-500]
        ...
        optimum_times = 0.0045*size[np.argmin(losses, axis=0)]
        ...
        if np.linalg.norm(y[500:len(time_og)-500] - e) < np.linalg.norm(y[500:len(time_og)-500] - final_estimate):

            optimum_times = (0.0045*size[best])*np.ones_like(optimum_times[500:len(time_og)-500])
            optimum_kp = (Kp[best,:])
            optimum_kd = (Kd[best,:])
        ...
        # plt.axhline((np.linalg.norm(y[500:len(time_og)-500] - final_estimate))/len(y[500:len(time_og)-500]))
        # plt.show()
        ...
        fig.suptitle('Estimates or gain = '+str(gain)+' and trial = '+str(trial))


        axs[0].plot(time[500:len(time_og)-500],y[500:len(time_og)-500], label = 'filtered step data')
        axs[0].plot(time[500:len(time_og)-500], e, label = 'estimate at timescale - ' + str(0.0045*size[best]))
        axs[0].legend()
        axs[1].plot(time[500:len(time_og)-500], y[500:len(time_og)-500], label = 'filtered step position data')
        axs[1].plot(time[500:len(time_og)-500], final_estimate,label = 'new step estimate', color = 'r')
        axs[1].legend()
        ...
        residuals = y[500:len(time_og)-500] - final_estimate
        # Fit a normal distribution to the residuals - note not always normal
        (mu, sigma) = norm.fit(residuals)
        ...
        # axs[0].plot(time[500:4500], e[500:4500], label = 'best estimate at constant timescale - ' + str(size[best]))
        # axs[0].legend()
        ...
        if np.linalg.norm(y[500:len(time_og)-500] - e) < np.linalg.norm(y[500:len(time_og)-500] - final_estimate):
            ...
            timescales_across_trials.append((0.0045*size[best])*np.ones_like(optimum_times[500:len(time_og)-500]))
            Kps_across_trials.append(Kp[best,500:len(time_og)-500])
            Kds_across_trials.append(Kd[best,500:len(time_og)-500])

        else: 
            timescales_across_trials.append(optimum_times[500:len(time_og)-500])
            Kps_across_trials.append(optimum_kp[500:len(time_og)-500])
            Kds_across_trials.append(optimum_kd[500:len(time_og)-500])

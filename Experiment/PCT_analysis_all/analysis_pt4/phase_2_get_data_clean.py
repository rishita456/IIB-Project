...
for i in range(len(time_data)):
    ...
    if pct == 0:
        axs.plot(x, pdf, linewidth=2, label='gain:'+str(gain) + ' trial:' + str(i) + ' tau:' + str(opt_time[0]), linestyle = '--')
    else:
        axs2.plot(x, pdf, linewidth=2, label=str(gain), c = 'red', alpha = ((i+1)/(len(time_data)+1)))

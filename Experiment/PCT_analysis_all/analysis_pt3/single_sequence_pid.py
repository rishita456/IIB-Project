...
def calc_func_find_Kp_0_part0(K):
    ...
    return np.linalg.norm((-y) + (Kp_0*((pg)**-1)*theta[0:-1])) #+ Kd_0*((pg)**-1)*theta_steps[0:-1] + Ki_0*((pg)**-1)*theta_integ[0:-1]) )
    ...
    for j in range(len(size)):
        ...
        for i in range(len(limits)-1):
            ...
            for i in range(len(detrended_pos)-1):
                ...
                theta_integ.append(sum(theta[0:i+1]))
            ...
            theta_integ = np.array(theta_integ)
            theta_integ = np.insert(theta_integ, -1,theta[-1])
    

            result = minimize(calc_func_find_Kp_0_part0, (1,1,1))
            print(result.x)
            loss.append(result.fun)
            
            est.append(-(result.x[0])*((gain)**-1)*theta)# - result.x[0]*((gain)**-1)*theta_steps -(result.x[2])*((gain)**-1)*theta_integ)
            
            Kps.append(result.x[0])
            Kds.append(result.x[1])
            Kis.append(result.x[2])
        ...
        Ki[j, 0:len(np.repeat(Kis, size[j]))] = np.repeat(Kis, size[j])
...
for i in range(5000):
    ...
    optimum_ki[i] = Ki[min_loss_indices[i], i]

final_estimate = optimum_kp[500:4500]*((traing)**-1)*theta[500:4500] #+ optimum_kd[500:4500]*((traing)**-1)*theta_steps[500:4500]  + optimum_ki[500:4500]*((traing)**-1)*theta_integ[500:4500]
...
plt.plot(time[500:4500], optimum_ki[500:4500])
plt.title('Optimum Ki values for gain = '+ str(gain))
plt.xlabel('time(s)')
plt.ylabel('Ki')
plt.show()
...
print(np.linalg.norm(y[500:4500]-final_estimate))
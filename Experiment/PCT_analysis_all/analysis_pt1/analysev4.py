...
def calc_func(time, theta, position, alpha, Kp_0, Kd_0, t_sep, gain1, gain2, c1, c2):
    ...
    # plt.plot(time,x, label = 'position estimate')
    # time = time - 0.15
    # plt.plot(time,position-750, label = 'actual position')
    # plt.legend()
    # plt.show()
...
print('Calibration index - ',(calibration_index[-1][-1]))
...
lr = 1
loss_sqrt, gamma_star, theta, dtheta, theta_est, gamma_true, x_t, time, g_t, dell  = calc_func(time, theta, position, 0.5, Kp_0, Kd_0, 12, 5, 10, 4, 4)
# plt.plot(gamma_star)
# plt.plot(gamma_true)
# plt.show()
...
#     loss_sqrt, gamma_star, theta, dtheta, theta_est, gamma_true, x_t, time, g_t, del_gamma_star  = calc_func(steady_state_calibration_time, steady_state_calibration_theta, position, alpha, Kp_0, Kd_0, 2, 1, 1)
...
# X, Y = np.meshgrid(x,x)
# loss = np.ones((len(x), len(x)))


# for iter in range(len(x)):
#     for iter2 in range(len(x)):

#         loss_sqrt, gamma_star, theta, dtheta, theta_est, gamma_true, x_t, time, g_t, dell  = calc_func(steady_state_calibration_time, steady_state_calibration_theta, steady_state_calibration_position, alpha, x[iter],x[iter2], 0, 1, 1)
#         loss[iter,iter2] = loss_sqrt**2
...
# ax3.legend()
# plt.show()
# plt.plot(steady_state_calibration_time, 100*theta_est, label = 'theta estimate')  

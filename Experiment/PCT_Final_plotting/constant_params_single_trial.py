...
def get_step_estimates_const(time, position, theta, gain, d, cutoff = 4, order = 5):
    ...
    final_estimate = Kps*((traing)**-1)*theta + Kds*((traing)**-1)*theta_steps_og
...
Kps, Kds, dist, final_estimate, steps = get_step_estimates_const(time_og, position_og, theta_og, gain, 1)



plt.plot(time_og, final_estimate, c = 'red', label = '$\hat{a_t}$')
plt.plot(time_og, steps, alpha = 0.4, c = 'blue', label = '$a_t$')
textstr = '     '.join(["$K_p = $" + str(round(Kps[10], 3)), "$K_d = $" + str(round(Kds[10], 3))])
props = dict(boxstyle='round', facecolor='orange', alpha=0.15)
plt.text(0.20, 0.8, textstr, transform=plt.gcf().transFigure, fontsize=10, bbox = props) 
plt.legend()
plt.xlabel('Time')
plt.ylabel("Pendulum angle (degrees)")
plt.title('Participant 3, phase A trial 7 - fit for constant parameters')
plt.show()

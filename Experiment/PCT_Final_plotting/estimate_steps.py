...
def get_step_estimates(time, position, theta, gain, d, cutoff = 4, order = 5, size = np.arange(200,501, 10)):
    ...
    traing = gain
    # window sizes over which parameters remain constant
    time_og = time
    ...
    # detrended_pos = [pos[i] for i in range(0, len(time))]
    # detrended_pos = position_og[0:len(detrended_pos)]
    # print(len(theta))

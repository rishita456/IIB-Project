...
for P_id in P_ids:
    ...
    if P_id == 4:
        training_gains = config.TRIAL_GAINS_4
    if P_id == 6:
        training_gains = config.TRIAL_GAINS_6
    if P_id == 7:
        training_gains = config.TRIAL_GAINS_7
    if P_id == 8:
        training_gains = config.TRIAL_GAINS_8
    if P_id == 9:
        training_gains = config.TRIAL_GAINS_9
    ...
    # opt_kp_values, opt_kp_samples, opt_kp_dist = get_noise_pdf_and_samples(Kps_stacked, len(opt_kp), min = two_std_range_kp[0], max=two_std_range_kp[1])
    # opt_kd_values, opt_kd_samples, opt_kd_dist = get_noise_pdf_and_samples(Kds_stacked, len(opt_kd), min = two_std_range_kd[0], max=two_std_range_kd[1])


    opt_filt_values, opt_filt_samples, opt_filt_dist = get_noise_pdf_and_samples(filt_res_stacked, 5000)
    # opt_fit_values, opt_fit_samples, opt_fit_dist = get_noise_pdf_and_samples(fit_res_stacked, 5000)

    # plt.plot(opt_kp_values, opt_kp_dist/np.sum(opt_kp_dist), label = 'Participant' + str(P_id))
    # plt.plot(opt_kd_values, opt_kd_dist/np.sum(opt_kd_dist), label = 'Participant' + str(P_id))
    plt.plot(opt_filt_values, opt_filt_dist/np.sum(opt_filt_dist), label = 'Participant' + str(P_id))
    # plt.plot(opt_fit_values, opt_fit_dist/np.sum(opt_fit_dist), label = 'Participant' + str(P_id))


plt.legend()
plt.xlabel('Filter residuals')
plt.ylabel('PDF')
plt.title('Estimated $P(r_{filt})$ across participants')
plt.show()
